...
- `docs/source-log.md` の「確認状態」を AIエージェントの独自判断で `人間確認済み` に変更しない。AIが新規追加するエントリは必ず `AI記載` にとどめ、人間確認を促すコメントを残すこと
- **例外（人間確認結果の転記）**: ユーザーが以下の4点をすべて明示し、かつ「人間確認済みであること」を明確に指示した場合に限り、AIエージェントはその確認結果の転記として確認状態を `人間確認済み` に更新してよい
  1. 確認対象（条文名・判例名）
  2. 確認URL（一次資料の直接リンク）
  3. 確認日
  4. 確認者（人名またはアカウント）
- この例外を適用する場合、PR本文に「人間確認した対象・確認URL・確認日・確認者・未確認のまま残す項目」を必ず明記すること
- ユーザーが確認を明示していない条文・判例を、まとめて `人間確認済み` にしてはいけない
